...
// ForSlice adds patch operations to this set for adding new values and removing old values, at the specified path.
...
// ForMap adds patch operations to this set for replacing existing keys with new values and removing keys that no
// longer exist, at the specified path.
func (p *Set) ForMap(path string, currValues, desiredValues map[string]*string, addSupported bool) {
	fmt.Println("forpath", path, currValues, "--", desiredValues)
	var patchOps []*apigateway.PatchOperation
